...
|                 | `read(options)` / `read_stream(options)`                         | Record-read APIs. Dispatch on `options.query_type`. `read_stream` supports both snapshot and incremental. Per-slice streaming lives on `FileGroupReader`. |
